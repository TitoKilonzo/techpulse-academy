      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 28 }}>
        ...
        <div style={{ position: 'relative', maxWidth: 380 }}>
          ...
          <input
            ...
            style={{ width: '100%', background: '#fff', border: '1px solid #E5E7EB', borderRadius: 8, padding: '9px 12px 9px 32px', color: '#111827', fontSize: '0.875rem', outline: 'none' }}
          />
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {CATEGORIES.map(c => (
            <button key={c.id} onClick={() => setCat(c.id)} style={{
              ...
              background:   cat === c.id ? 'rgba(37,99,235,0.10)' : 'var(--bg-card)',
            ...
            return (
              <Link key={c.slug} href={`/dashboard/courses/${c.slug}`} className="course-card" style={{
                ...
                <div style={{ padding: '16px 18px' }}>
                  <h3 style={{ fontSize: '0.9rem', fontWeight: 600, marginBottom: 5, lineHeight: 1.35, color: '#111827' }}>{c.title}</h3>
                  <p style={{ fontSize: '0.78rem', color: '#4B5563', marginBottom: 12, lineHeight: 1.55, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                    {c.description}
                  </p>
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', fontSize: '0.72rem', color: '#9CA3AF', marginBottom: 10 }}>
                    ...
                    <span style={{
                      ...
                      background:  c.level === 'beginner' ? 'rgba(16,185,129,0.08)' : c.level === 'intermediate' ? 'rgba(245,158,11,0.08)' : 'rgba(239,68,68,0.08)',
                      color:       c.level === 'beginner' ? '#10B981' : c.level === 'intermediate' ? '#F59E0B' : '#EF4444',
                  ...
                  <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                    {c.tags.slice(0, 3).map(t => (
                      <span key={t} style={{ fontSize: '0.63rem', padding: '2px 8px', borderRadius: 100, background: 'var(--bg-hover)', border: '1px solid #E5E7EB', color: '#9CA3AF' }}>{t}</span>
                    ))}
                  </div>
                </div>
              </Link>
