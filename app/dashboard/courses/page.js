...
function CoursesList() {
  ...
  // Sync state from URL on mount and param change
  useEffect(() => {
  ...
  const handleCatChange = (c) => { setCat(c); updateUrl('cat', c); };
  const handleLevelChange = (l) => { setLevel(l); updateUrl('level', l); };
  const handleSearchChange = (q) => { setSearch(q); updateUrl('q', q); };
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {CATEGORIES.map(c => (
            <button key={c.id} onClick={() => handleCatChange(c.id)} style={{
              ...
              background:   cat === c.id ? 'var(--bg-hover)' : 'var(--bg-card)',
              color:        cat === c.id ? 'var(--orange-dark)' : 'var(--text-secondary)',
              borderColor:  cat === c.id ? 'var(--orange)' : 'var(--border)',
            }}>{c.label}</button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {['all', 'beginner', 'intermediate', 'advanced'].map(l => (
            <button key={l} onClick={() => handleLevelChange(l)} style={{
              ...
              background:  level === l ? 'var(--bg-hover)' : 'var(--bg-card)',
              color:       level === l ? 'var(--orange-dark)' : 'var(--text-muted)',
              borderColor: level === l ? 'var(--orange)' : 'var(--border)',
            }}>{l}</button>
          ))}
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 18 }}>
          {filtered.map(c => {
            ...
            return (
              <Link key={c.slug} href={`/dashboard/courses/${c.slug}`} className="card-premium course-card" style={{
...
export default function CoursesPage() {
  return (
    <Suspense fallback={<div className="skeleton" style={{ height: 400, width: '100%' }} />}>
      <CoursesList />
    </Suspense>
  );
}
